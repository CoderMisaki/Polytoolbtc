...
const { publicError, updatePosition } = require('./_positions');

module.exports = async function handler(req, res) {
  setCors(req, res);
  res.setHeader('Allow', 'PATCH, POST, OPTIONS');
  if (req.method === 'OPTIONS') return res.status(204).end();
  if (req.method !== 'PATCH' && req.method !== 'POST') return publicError(res, 405, 'Method Not Allowed');

  try {
    const { userId } = await requireAuth(req);
    const rateLimit = await checkRateLimit(req, { userId, route: 'update-position', limit: 30, windowMs: 60_000, failClosed: true });
    applyRateLimitHeaders(res, rateLimit);
    if (!rateLimit.allowed) return publicError(res, 429, 'Terlalu banyak request. Coba lagi sebentar lagi.');
    const result = await updatePosition(userId, req.body);
